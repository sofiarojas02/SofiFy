...
function LibraryHeader({ onAddItem, items }){
    const [libraryAdded, setLibraryAdded] = React.useState(false)
    const [libraryModal, setLibraryModal] = React.useState(false)
    const [libraryModalType, setLibraryModalType] = React.useState('')
    const [formData, setFormData] = React.useState({ title: '', subtitle: '', img: '' })
    const [typeError, setTypeError] = React.useState('')

    const plusRef = React.useRef(null)
    const libraryModalRef = React.useRef(null)
    ...
    const addItemToLibrary = (newItem)=>{
        if(newItem.title.trim() === ''){
            ...
            onAddItem({...newItem, type: libraryModalType})
            setLibraryModal(false)
            setFormData({title: '', subtitle: '', img: ''})
        }
    }
                    ...
                    {libraryModal && (
                        ...
                        className='libraryModal__form p-3 bg-dark position-absolute m-3 rounded-2'>
                            ...
                            className='d-flex flex-column'>
                                ...
                                <div className={`libraryModal__element ${libraryModalType === 'artista' && 'd-none'}`}>
